...
export class SeatService {
  ...
  private fetchSeats(showtimeId: string): Observable<Seat[]> {
    ...
    return this.http.get<Seat[]>(`${environment.apiUrl}/seats/showtime/${showtimeId}?_t=${timestamp}`);
  }
  ...
  private buildSeatLayout(showtimeId: string, seats: Seat[]): SeatLayout {
    ...
    return {
      ...
      availableSeats: seats.filter(s => !s.isBooked && !s.isHeld && !s.isDisabled).length,
      seats: seats.map(s => ({
        id: s.id,
        row: s.row,
        number: s.number,
  ...
  getRealTimeAvailability(showtimeId: string): Observable<SeatAvailability[]> {
    const timestamp = new Date().getTime();
    return this.http.get<Seat[]>(`${environment.apiUrl}/seats/showtime/${showtimeId}?_t=${timestamp}`).pipe(
      map(seats => seats.map(seat => ({
        seatId: seat.id,
        isAvailable: !seat.isBooked && !seat.isHeld && !seat.isDisabled,
